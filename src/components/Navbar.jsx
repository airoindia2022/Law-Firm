        ...
        return (
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-[90vw] max-w-5xl bg-white rounded-3xl shadow-2xl overflow-hidden opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-500 z-50 border border-black/5 flex h-[480px]">
                ...
                <div className="w-1/4 bg-slate-50 p-6 border-r border-black/5">
                    <h3 className="text-[10px] font-bold text-brand uppercase tracking-[0.3em] mb-6">Specializations</h3>
                        ...
                        <AnimatePresence mode="wait">
                            ...
                            >
                                {filteredItems.map((sub) => (
                                    ...
                                    >
                                        <span className="text-[13px] font-bold text-slate-800 group-hover/sub:text-brand transition-colors">
                                            {sub.name}
                                        </span>
                                        {sub.description && (
                ...
                <div className="w-1/5 relative overflow-hidden group/cta">
                    ...
                    <div className="absolute inset-0 p-6 flex flex-col justify-end">
                        ...
                        <h4 className="text-base font-serif font-bold text-white mb-2 leading-tight">Elite Legal Representation</h4>
                        <p className="text-white/80 text-[10px] mb-4 font-medium">Trusted by Fortune 500 companies.</p>
                        <Link to="/contact" className="inline-flex items-center gap-2 text-white text-[10px] font-bold uppercase tracking-widest hover:gap-4 transition-all">
                        ...
                        <div className="hidden lg:flex items-center space-x-0.5 xl:space-x-1">
                            {MENU_ITEMS.map((item) => (
                                <div key={item.name} className="relative group px-3 xl:px-4 py-2">
                                    <Link
                                        to={item.path}
                                        className={`relative py-2 text-[11px] xl:text-[12px] font-bold uppercase tracking-[0.12em] xl:tracking-[0.15em] transition-all duration-300 ${linkClass} group-hover:text-brand`}
                                    >
                                        {item.name}
                        ...
                        >
                                                        ...
                                                        <div className="pb-4 pl-3 grid gap-2">
                                                            {item.submenu.map((sub) => (
                                                                <Link
                                                                    ...
                                                                    to={sub.path}
                                                                    onClick={() => setIsOpen(false)}
                                                                    className="flex items-center gap-3 py-2 px-3 rounded-lg text-sm font-medium text-slate-600 hover:text-brand hover:bg-slate-50 transition-all"
                                                                >
                            ...
                            <div className="p-5 bg-slate-50 border-t border-slate-100 flex-shrink-0">
                                <Link
                                    to="/contact"
                                    onClick={() => setIsOpen(false)}
                                    className="flex items-center justify-center gap-3 w-full py-4 bg-brand text-white rounded-2xl text-sm font-bold uppercase tracking-widest shadow-lg shadow-brand/20 active:scale-[0.98] transition-all hover:bg-[#8B6508]"
                                >
